...
<body class="flex items-center justify-center min-h-screen p-4">
    ...
    <script type="module">
        ...
        // ЕСКЕРТУ: Егер сіз бұл файлды Canvas платформасынан тыс іске қоссаңыз (локальді түрде), 
        // __firebase_config айнымалысы анықталмайды. Дерекқорды локальді пайдалану үшін, 
        // төмендегі `{}` орнына өз Firebase жобаңыздың конфигурациясын қолмен енгізіңіз.
        ...
        window.gameState = {
            ...
            isDbConnected: false // New state for tracking DB connection status
        };
        ...
        // --- GEMINI API FUNCTIONS (Көп өзгеріссіз) ---
        ...
        async function initializeFirebase() {
            try {
                // Check if firebaseConfig has necessary keys (like projectId) to be considered valid
                const hasValidConfig = firebaseConfig && firebaseConfig.projectId;

                if (!hasValidConfig) {
                    console.error("Firebase конфигурациясы жеткіліксіз. Canvas сыртында іске қосу үшін қолмен конфигурациялау қажет.");
                    
                    // Display specific error message for local run
                    document.getElementById('app').innerHTML = `
                        <div class="text-center p-10 bg-gray-800 rounded-xl shadow-2xl w-full max-w-md">
                            <h1 class="text-2xl font-bold text-red-400 mb-4">Дерекқор Қатесі (Локальді Іске Қосу)</h1>
                            <p class="mb-6 text-gray-200">Бұл қосымша нәтижелерді сақтау үшін Firebase-ді қолданады.</p>
                            <p class="mb-6 text-yellow-300">Сіз бұл файлды тікелей ашқандықтан, Firebase конфигурациясы жоқ. Егер сіз нәтижелерді сақтағыңыз келсе, кодтағы <code>firebaseConfig</code> айнымалысына өз Firebase жобаңыздың конфигурациясын қолмен енгізіңіз. Әйтпесе, тестті дерекқорсыз жалғастыра аласыз (нәтижелер сақталмайды).</p>
                            <button onclick="window.gameState.isAuthReady = true; window.updateUI();" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full shadow-lg transition duration-150 w-full">
                                Тестті Дерекқорсыз Бастау
                            </button>
                        </div>
                    `;
                    // Set auth ready state but proceed without DB setup
                    window.gameState.isAuthReady = true;
                    window.gameState.isDbConnected = false;
                    window.updateUI(); 
                    return;
                }

                // If configuration is valid, proceed with initialization
                ...
                await new Promise((resolve, reject) => {
                    const unsubscribe = onAuthStateChanged(auth, async (user) => {
                        ...
                        window.gameState.isAuthReady = true;
                        window.gameState.isDbConnected = true; // DB is connected
                        window.updateUI(); 
                        resolve();
                    }, reject);
                });

                if (window.gameState.isAuthReady && window.gameState.isDbConnected) {
        ...
        window.googleSignIn = async function() {
             if (!window.gameState.isDbConnected) {
                alert("Дерекқор қосылмаған. Кіру мүмкін емес.");
                return;
            }
        ...
        async function saveResult() {
            if (!userId || !window.gameState.isDbConnected || userId.startsWith('anon')) {
                console.warn("Нәтиже сақталмайды: Дерекқор қосылмаған немесе қолданушы анонимді.");
                return;
            }
        ...
        function setupLeaderboardListener() {
            if (!db || !window.gameState.isAuthReady || !window.gameState.isDbConnected) {
        ...
        async function fetchAllResultsForExport() {
             if (!db || !window.gameState.isDbConnected) {
                console.warn("Экспорттау мүмкін емес: Дерекқор қосылмаған.");
                return [];
            }
            // FIX: Using the public data path
            const resultsCollectionRef = collection(db, 'artifacts', appId, 'public', 'data', 'quiz_results');
        ...
        window.startQuiz = function() {
            ...
            if (isAnonymous && window.gameState.isDbConnected) {
                // Show custom modal/message for sign-in requirement if DB is connected
                document.getElementById('app').innerHTML = `
                    <div class="text-center p-10 bg-gray-800 rounded-xl shadow-2xl w-full max-w-md">
                        <h1 class="text-2xl font-bold text-red-400 mb-4">Кіру Қажет</h1>
                        <p class="mb-6 text-gray-400">Нәтижелеріңізді сақтау және тестті бастау үшін Google аккаунты арқылы кіріңіз.</p>
        ...
        window.handleAnswer = function(selectedIndex) {
            ...
            setTimeout(() => {
                ...
                const isFinal = window.gameState.currentQuestionIndex >= QUESTIONS.length;
                ...
                // Save result if not anonymous and DB is connected
                if (window.gameState.isDbConnected) {
                    saveResult();
                }
                window.updateUI();
        ...
        window.exportResults = async function() {
            if (!db || !window.gameState.isDbConnected) {
                alert("Дерекқор қосылмаған. Экспорттау мүмкін емес.");
                return;
            }
        ...
        window.updateUI = function() { 
            ...
            if (window.gameState.isDbConnected) {
                authDiv.innerHTML = !isAnonymous ? 
                 ...
                 authDiv.innerHTML = `<span class="text-sm text-red-400 font-medium">Дерекқор Өшірулі</span>`;
            }
        ...
        window.renderTeacherView = function() { 
            ...
            const dbWarning = !window.gameState.isDbConnected 
                ? `<div class="p-3 bg-red-900 rounded-lg text-sm text-red-200 mb-4"><i class="fas fa-exclamation-triangle mr-2"></i> Дерекқор қосылмаған. Нәтижелерді көру және экспорттау мүмкін емес.</div>`
                : '';
                ...
                <div class="w-full max-w-4xl p-6 bg-gray-800 rounded-2xl shadow-2xl text-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <button onclick="window.exportResults()" 
                            class="bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-xl shadow-md transition duration-150 ${!window.gameState.isDbConnected ? 'opacity-50 cursor-not-allowed' : ''}" 
                            ${!window.gameState.isDbConnected ? 'disabled' : ''}>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-gray-700 rounded-xl shadow-lg">
                            ...
                            <tbody>
                                ${results.length === 0 ? 
                                    `<tr><td colspan="5" class="p-4 text-center text-gray-400">${window.gameState.isDbConnected ? 'Әлі нәтижелер жоқ.' : 'Дерекқор қосылмаған.'}</td></tr>` : 
        ...
        window.renderStudentView = function() { 
            const isAnonymous = userId && userId.startsWith('anon');
            const canStartQuiz = !isAnonymous || !window.gameState.isDbConnected;
            ...
            if (!window.gameState.isQuizActive && !window.gameState.quizComplete) {
                ...
                if (!window.gameState.isDbConnected) {
                     startButtonHTML = `<button onclick="window.startQuiz()" class="bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xl py-4 px-10 rounded-xl shadow-xl transition duration-200 transform hover:scale-105 w-full">
                           Дерекқорсыз Бастау (Сақталмайды) <i class="fas fa-play ml-2"></i>
                       </button>`;
                } else if (isAnonymous) {
                     startButtonHTML = `<button onclick="window.startQuiz()" class="bg-yellow-600 hover:bg-yellow-700 text-white font-extrabold text-xl py-4 px-10 rounded-xl shadow-xl transition duration-200 transform hover:scale-105 w-full">
                           Қонақ Режимінде Жалғастыру
                       </button>`;
                } else {
                    startButtonHTML = `<button onclick="window.startQuiz()" class="bg-green-600 hover:bg-green-700 text-white font-extrabold text-xl py-4 px-10 rounded-xl shadow-xl transition duration-200 transform hover:scale-105 w-full">
            ...
            if (window.gameState.quizComplete) {
                return `
                    <div class="text-center p-10 bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg">
                        ...
                        ${!window.gameState.isDbConnected ? 
                            '<p class="text-red-400 text-sm mb-4">Ескерту: Дерекқор қосылмағандықтан, нәтижеңіз сақталмады.</p>' : ''}
